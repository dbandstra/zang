...
fn State(comptime Writer: type) type {
    return struct {
        ...
        fn printExpressionResult(self: *@This(), result: ExpressionResult) Writer.Error!void {
            switch (result) {
                .nothing => unreachable,
        ...
        fn printFloatDest(self: *@This(), dest: FloatDest) !void {
            try self.print("temp_float{usize}", .{dest.temp_float_index});
        }

        fn printBufferDest(self: *@This(), dest: BufferDest) !void {
            switch (dest) {
        ...
        fn indent(self: *@This(), indentation: usize) !void {
            var i: usize = 0;
            ...
            switch (instr) {
                ...
                .track_call => |track_call| {
                    ...
                    for (track_call.instructions) |sub_instr| {
                        try self.printInstruction(sub_instr, indentation + 1);
                    }
                },
                .delay => |delay| {
...
pub fn printBytecode(out: anytype, cs: *const CodegenState, cms: *const CodegenModuleState) !void {
    ...
    for (cms.instructions.items) |instr| {
        try self.printInstruction(instr, 1);
    }
    try self.print("\n", .{});
